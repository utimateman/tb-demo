...
raw_user_activity_df = spark.sql(
    """
    SELECT *
    FROM delta.`s3a://""" + bucket_name + "/raw_user_activity_data`"
)
# raw_user_activity_df = processed_user_level_df.withColumnRenamed("user_id","id")
raw_user_activity_df.show() 
...
df_inner = processed_user_df.join(raw_user_activity_df, on=['id'], how='inner')
df_inner.show()
...
bucket_name = "demo-bucket"
df_inner.write.format("delta").mode("append").option("mergeSchema", "true").save("s3a://" + bucket_name + "/detail_user_data")
print("[ successful ] - write detail_user_data")
